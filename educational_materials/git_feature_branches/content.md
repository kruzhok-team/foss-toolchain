...
Первой функцией, которую нужно было добавить, была запрос для возврата списка 
...
Алиса протестировала новый запрос локально, чтобы убедиться, что она работает 
...
* Описание: Этот запрос на слияние добавляет новую запрос (/items) для 
возврата списка предопределенных элементов. После создания запроса на слияние 
она назначила Федора рецензентом.

### Принятие изменений

Федор получил уведомление о запросе на слияние Алиса. Он просмотрел изменения кода 
...
Удовлетворенный реализацией Алисы, Федор одобрил запрос на слияние и объединил его 
...
Теперь очередь Федор поработать над функцией. Он решил добавить запрос для извлечения
...
## Разгребаем конфликты

По мере того, как Алиса и Федор продолжали работать над своим веб-приложением Python, они столкнулись 
